...
class DownloadTemplateTest  extends IntegrationTest {
    ...
    def "should download template for proper hashcode"() {
        ...
        when:
        ResponseEntity response = restTemplate.getForObject('http://localhost:8081/pobierz/{template}/{cms}/{hash}/{timestamp}', ResponseEntity.class, 'T11', 'WORDPRESS', hashSend, timestamp)
    ...
    def "should not download template for expired hashcode"() {
        ...
        when:
        def response = restTemplate.getForObject('http://localhost:8081/pobierz/{template}/{cms}/{hash}/{timestamp}', String.class, 'T11', 'WORDPRESS', hashSend, timestamp);
    ...
    def "should not download template for bad template"() {
        ...
        when:
        def response = restTemplate.getForObject('http://localhost:8081/pobierz/{template}/{cms}/{hash}/{timestamp}', String.class, 'bad_template', 'WORDPRESS', hashSend, timestamp);
    ...
    def "should not download template for bad cms"() {
        ...
        when:
        def response = restTemplate.getForObject('http://localhost:8081/pobierz/{template}/{cms}/{hash}/{timestamp}', String.class, 'T11', 'bad_cms', hashSend, timestamp);
